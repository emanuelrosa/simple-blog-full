...
class ConfigDao {
    ...
    function getConfig() {
        try {
            ...
            $c = new Config();
            foreach ($stmt as $row) {
                $c->setAllAtributes($row['idconfig'], $row['gifload'], $row['imglogo'], $row['imgtopo'], $row['imgsocial'], $row['imglogoadm'], $row['titulo'], $row['descricao'], $row['autor'], $row['header'], $row['body'], $row['palavra_chave'], $row['appid_facebook'], $row['num_comentarios_visivel']);
            }
    ...
    function editConfigComentarioFacebookConfig(Config $c) {
        try {
            $stmt = $this->p->prepare("UPDATE `config` SET `appid_facebook` = ?,
                `num_comentarios_visivel` = ? WHERE `idconfig` = ?");
